...
public class SseController {
    ...
    public SseEmitter subscribe(@PathVariable("workspaceId") Long workspaceId,
        ...
        sseService.sendEmitter(emitter);

        return emitter;
    }

    // 특정 페이지 변화 감지
    @GetMapping("/{workspaceId}/page/{page}/subscribe")
    public SseEmitter subscribePage(@PathVariable Long workspaceId,
                                    @PathVariable String page,
                                    @RequestParam("token") String token) {
        log.info("== SSE 페이지 구독 API 진입 ==");

        if (token == null || !jwtTokenProvider.validateToken(token)) {
            throw new IllegalArgumentException("유효하지 않은 토큰입니다.");
        }

        log.info("페이지 구독 token: {}", token);
        ...
        sseEmitterRepository.savePage(workspaceId, page, userId, emitter);

        emitter.onCompletion(() -> sseEmitterRepository.deletePage(workspaceId, page, userId));
        emitter.onTimeout(() -> sseEmitterRepository.deletePage(workspaceId, page, userId));
        emitter.onError((e) -> sseEmitterRepository.deletePage(workspaceId, page, userId));
